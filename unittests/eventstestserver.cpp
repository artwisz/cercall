...
public:
    ...
    void trigger_single_broadcast(std::unique_ptr<EventType> e) override
    {
        myEnabledEvent = std::move(e);
...
private:
    ...
    asio::steady_timer myEventTimer;
    std::unique_ptr<EventType> myEnabledEvent;

    void tickTimer(const cercall::asio::ErrorCode& ec)
    {
        if (ec != asio::error::operation_aborted && myEnabledEvent.get() != nullptr) {
            /* This is not a practical use case - usually events will be generated by the service, instead of
             * recasting something received from the client as done below. */
            std::string eventClassName;
            if (myEnabledEvent->get_as<RealEventClassOne>() != nullptr) {
                eventClassName = "RealEventClassOne";
                broadcast_event<RealEventClassOne>(myEnabledEvent->get_as<RealEventClassOne>()->myEventData);
                ...
                broadcast_event<RealEventClassThree>(myEnabledEvent->get_as<RealEventClassThree>()->myEventDict);
            } else if (myEnabledEvent->get_as<RealEventClassTwo>() != nullptr) {
                eventClassName = "RealEventClassTwo";
                broadcast_event<RealEventClassTwo>(myEnabledEvent->get_as<RealEventClassTwo>()->myEventData);
            }
